...
## For other services calling this API: use `/sync_account_history` — there is no `/login` anymore
...
- The standalone `/login` and `/logout` endpoints (`routes/auth.py`) were removed — `/sync_account_history` was already the only race-free way to authenticate, so the raw endpoints existed only as a footgun. Login now happens exclusively inside `sync_worker.process_sync_job` via the single-call `mt5.initialize(login=, password=, server=)`, which both starts the terminal (if needed) and authorizes in one IPC round trip — no separate `mt5.initialize()` + `mt5.login()` pair, and no bare `mt5.initialize()` bootstrap to depend on either.
...
- Every other pre-existing route (`/history_deals_get`, `/symbol_info_tick`, `/get_positions`, etc.) bypasses this queue entirely and is **not** synchronized against it or against each other — they only work at all because some earlier `/sync_account_history` call already logged the process into an account. If another API needs login+history data, it should go through `/sync_account_history` exclusively rather than mixing in the raw endpoints.
